...
yta = yt(:,[15:78]);
ytb = yt(:,[17:80]);% removing cyclic prefix
% converting to frequency domain
yFa = (sqrt(nDSC)/nFFT)*fftshift(fft(yta.')).';
yFb = (sqrt(nDSC)/nFFT)*fftshift(fft(ytb.')).';
yModa = yFa(:,[6+[1:nBitPerSym/2] 7+[nBitPerSym/2+1:nBitPerSym] ]);
yModb = yFb(:,[6+[1:nBitPerSym/2] 7+[nBitPerSym/2+1:nBitPerSym] ]);
% BPSK demodulation
% +ve value --> 1, -ve value --> -1
ipModHat = 2*floor(real(yModa/2)) + 1;
...
%Scatterplot
scatterplot(ipBit); %sebelum modulasi
B=ipMod(1,:);
scatterplot(B);
A=yModa(1,:);
scatterplot(A)
